...
8. 目标节点中序遍历的后一个节点
	* 分类要清晰
		1. 目标节点右子树不为空: 返回右子树最左的节点。这是非递归遍历二叉树的一个重要步骤，当访问完目标节点后，然后一路沿着目标节点右子树的左分叉一路访问到一个没有左子树的节点，下一轮访问的时候首先弹栈，访问的就是右子树上最左侧的节点。
		2. 目标节点右子树不为空: 向上回溯，找到第一个目标节点处于其左子树上的节点返回即可。联系非递归中序遍历二叉树的过程，如果目标节点右子树为空，在访问过程中，如果目标节点不是二叉树的最右侧节点，那么该节点一定至少在一个节点的左子树上，找到第一个目标节点处于其左子树上的节点就是中序遍历的下一个节点
9. 双栈实现队列，双队列实现栈
0. 斐波那契数列
	* 用pre, cur, tmp即可实现
	* **最后返回pre**
1. 旋转数组查找：给定旋转数组$A$
	* 主要性质：旋转后数组分为两个递增数组：**前端**和**后段**，且前段最小值大于后段递的最大值。
	* 查找旋转数组的最小值：BS， 当$A_{mid} \geq A_0$时，`low = mid+1`。
	* 查找指定$target$：先根据$A_{mid}$和$A_{low}$的关系判定$A_{mid}$处于前段还是后段
		* $A_{mid} >= A_{low}$：$A_{mid}$处于前段
			* $target\in[A_{low}, A_{mid})$:`high = mid - 1`
			* $target \notin [A_{low}, A_{mid})$`：low = mid + 1`。
		* $A_{mid} < A_{low}$：$A_{mid}$处于后段
			* $target \in (A_{mid}, A_{high}]$：`low = mid + 1`
			* $target \notin (A_{mid}, A_{high}]$：`high = mid - 1`。
			
2. 字符矩阵中找字符串的路径
	* 每个元素作为起点进行DFS，状态需要回溯
3. 机器人能够到达的位置
	* Leetcode Maze，障碍单元格标记为`'X'`，空当单元格为空字符`' '`，从起点开始DFS，访问之后将单元格记为`'0'`，状态不需要回溯，DFS结束后统计`'0'`的个数即可。
4. **割绳子**
	* dp
...
5. 数的二进制表示中有多少个1
	...
	* 原数和原数减一相与，看能进行多少次这种操作！
		* `x &= x-1`作为一个基本操作，无论x的正负，x都会逐渐收敛到0，这个方法非常tricky！也可以用这个方式判定一个数是否为2的指数幂。
	* 直接构造`bitset`对象`o`并调用`o.count()`即可。
6. double power:给定`double`型底数$base$和`int`型指数$e$，可以使用位运算
	* $base^{2i} = base^{i} * base^{i}$
	* 将$e$视为二进制表示，每位的权值为$i$，其高位的权值为$2i$，基于这个规律使用右移操作由低位遍历到高位即可。
7. 顺序打印从1到n位最大整数
	* 大数使用字符串表示
...
2. 二叉树层序遍历
	...
	* 之字层序遍历:BFS
		...
		双deque实现BFS的过程中，读写方向的转换也很有趣，有一种物理上模拟了“之”字遍历的既视感：cur头next尾+cur尾next头或者cur尾next头+cur头next尾
3. 二叉树后序遍历
	* 非递归方法
