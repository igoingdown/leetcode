...
BS、TP。二分法没什么说的。TP将问题转化为链表内部环检测的问题。
...
DP。给定K,N,W，当牌的点数小于K时，从[1, W]的牌中等概论随机抽取一张，计算手牌的面值之和<=N的概率。使用pre记录当前牌面值总和之前的概率总，相当于前缀和，使用dp记录当前牌面值的概率。dp[i]最多只跟[i - 1, i-1-W]的牌面值概率的总和sum有关，dp = sum / W, 使用b记录相关牌面值的起点，使用e记录牌面值的终点。每次计算dp之后，pre[i + 1] = pre[i] + dp[i]。
...
730. Count Different Palindromic Subsequences
    ...
    1. $s[i] == s[j]$:
      ...
      假设$s[i+1:j-1]$的不重复的回文子序列为$\{y_1, y_2, ..., y_k\}$，$s[i]=s[j]=a$，不考虑具体的细节粗略估计应该有$DP[i][j]=DP[i+1][j-1] * 2 + 2$，这是因为$s[i:j]$的不重复回文子串粗略想想应该有$\{y_1, y_2, ..., y_n, ay_1a, ay_2a, ..., ay_na, a, aa\}$，下面考虑可能出现的重复，当$s[i:j]$的模式为`axxxaxxxaxxxa`时，就会出现重复，因此可以用TP的方法，分别从$s[i+1]$和$s[j-1]$向内查找$a$，设出现的位置分别为$l$和$r$。
...
142. Linked List Cycle II
    ...
    * 引申的链表相交问题：
      ...
      ​	a. 两个都没环：判断两者最后一个节点是否相同
      	b. 一个有环，一个没环，不可能相交
      	c. 两个都有环，看一个链表的碰撞点是否在另一个链表的环内，因为若相交，环一定共享！
